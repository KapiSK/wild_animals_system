...
\begin{abstract}
Wildlife damage to agricultural crops is a severe issue in Japan, increasing the demand for real-time trail camera monitoring. However, conventional communication-based cameras face challenges regarding initial and operational costs, as well as a high volume of "empty shots" triggered by environmental factors such as wind. To address these issues, we propose a low-cost, three-layer monitoring system comprising edge cameras, an edge server, and a cloud server. By building a local area network between the cameras and the edge server, communication lines are aggregated, thereby suppressing operational costs. Furthermore, the system employs a two-stage animal detection process: a lightweight AI (YOLOv8n) at the edge filters out empty shots to reduce image transmission, while a large-scale AI in the cloud performs highly accurate final judgments to prevent false alarms. Experiments demonstrated that the proposed system reduced the number of transmitted images by approximately 81.2\% during the daytime and 50.7\% at night.
\end{abstract}
...
\begin{figure}[tb]
    \centerline{\includegraphics[width=0.8\linewidth]{../img/system_structure_eng.png}}
...
\begin{figure}[tb]
    \centerline{\includegraphics[width=\columnwidth]{../img/system_flow_eng.png}}
...
\section{Conclusion}
We proposed a low-cost monitoring system for wildlife damage control. It uses a three-layer structure: edge cameras, an edge server, and a cloud server. This reduces costs and workload. This system builds a local network between the edge cameras and the edge server. This eliminates the need for individual communication line contracts for each camera. We demonstrated an architecture that aggregates the communication of multiple cameras into a single line, suppressing operational costs. Our verification experiments combined primary selection (YOLOv8n) and cloud re-judgment. We confirmed image reduction rates of 81.2\% during the daytime and 50.7\% at night.
...
This section has been removed for the double-blind review process.
