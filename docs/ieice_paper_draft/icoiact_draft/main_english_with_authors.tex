...
Regarding research emphasizing real-time performance, Whytock et al. \cite{Whytock2023} developed an instant alert system using satellite communication. However, the communication costs are extremely high, posing a significant barrier to widespread adoption by general farmers and local governments. As attempts to perform inference on edge devices, Patkar et al. \cite{Patkar2025} and Rahman et al. \cite{Computers2025} proposed high-performance monitoring systems using single-board computers like Raspberry Pi. In particular, Patkar et al. removed empty shots using YOLOv8n on the edge side, demonstrating that high-speed inference is possible even on inexpensive devices. Recently, Kathir et al. \cite{10716895} and Selvaraj et al. \cite{11459678} have similarly proposed YOLO-based animal intrusion detection and alert systems, and the effectiveness of edge AI in wildlife damage control around agricultural lands has been widely confirmed. However, these devices have relatively high power consumption, and long-term operation in mountainous areas without commercial power requires large batteries or solar panels, increasing equipment costs and installation difficulty.

As an approach to power saving and communication cost reduction, Sato et al. \cite{Sato2022} proposed a network architecture consisting of multiple child nodes and one parent node. This method showed the effect of reducing the operational cost and power consumption of the entire system by aggregating communication lines at the parent node. To reduce system costs and communication delays, Monburinon et al. \cite{8912138} proposed hierarchical edge computing that delegates main processing to gateway devices like Raspberry Pi, and Zualkernan et al. \cite{9345858} constructed an IoT architecture linking edge inference with a cloud DB. However, even in these methods, edge devices (child nodes) with camera functions often perform detection using deep learning, leaving the issue that each of the many deployed camera nodes still requires a certain amount of computational resources and power. Moreover, Saito et al. \cite{IEICE2020} proposed a node control method combining a power-saving microcontroller and a Raspberry Pi, and Semba et al. \cite{SMC2024} developed an automatic tracking device using deep learning, but these remain verified only in small-scale configurations, and the construction of a wide-area communication network in an actual field environment and the practical filtering effect of empty shots have not been sufficiently verified.

Furthermore, in recent years, research on wildlife damage control has progressed beyond image-based detection to more active countermeasures. For example, Kamesaka et al. \cite{8716170} developed a capture system that recognizes troops of monkeys using deep learning and automatically activates traps. Manavalasundaram et al. \cite{11324419} proposed a system that repels animals by emitting specific ultrasonic waves according to the detected animal species. The low-cost communication-based monitoring system proposed in this study is considered applicable as a sensor network to support operation in extensive agricultural lands by linking with these advanced damage deterrence and capture systems in the future.
...
\begin{figure}[tb]
    \centerline{\includegraphics[width=\linewidth]{../img/system_structure_eng.png}}
...
\begin{figure}[tb]
    \centerline{\includegraphics[width=0.45\linewidth]{../img/prototype_camera.jpeg}}
...
\begin{figure*}[tb]
    \centerline{\includegraphics[width=0.7\linewidth]{../img/system_flow.png}}
...
\subsection{Reduction Effect and Detection Accuracy}
...
\section{Conclusion}
In this paper, we proposed a low-cost, communication-based monitoring system with a three-layer structure consisting of edge cameras, an edge server, and a cloud server, aiming to reduce image review workload and initial/operational costs in wildlife damage control. We presented an architecture that constructs a local network between edge cameras and the edge server, eliminating the need for individual communication line contracts for each camera and aggregating communication from multiple cameras into a single line, thereby significantly suppressing operational costs. Furthermore, through verification experiments combining primary selection by YOLOv8n on the edge server and re-judgment on the cloud, we confirmed high communication reduction rates of approximately 81.2\% during the daytime and 50.7\% at night.
